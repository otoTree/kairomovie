...
export async function ensureCloudTables() {
  ...
  `);
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS api_canvas_histories (
      id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
      user_id uuid NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      project_id uuid NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
      name text NOT NULL,
  ...
  `);
  await db.execute(sql`
    CREATE UNIQUE INDEX IF NOT EXISTS uniq_api_canvas_histories_project_name
    ON api_canvas_histories(project_id, name)
  `);
  await db.execute(sql`
    CREATE INDEX IF NOT EXISTS idx_api_canvas_histories_user_time
    ON api_canvas_histories(user_id, updated_at)
  `);
  await db.execute(sql`
